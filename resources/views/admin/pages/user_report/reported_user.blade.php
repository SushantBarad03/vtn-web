    ...
    var table = $('#table_DT');
      oTable = table.DataTable({
        ...
        columns: [
            ...
            {"title": "User Name",'width':'15%', data:"name"},
            {"title": "Report Message",'width':'66%', data:"message"},            
            {"title": "Status",'width':'5%', data:"status"},
            {"title": "Date",'width':'5%', data:"date"},
            {"title": "Action",'width':'4%',"data":"action", searchble: false, sortable:false }, 
        ],
        responsive: false,
        order: [
          [0, 'desc']
        ...
        pageLength: 5,
        ajax: {
            url: "{{ route('complainant_data.userListing') }}", 
            data: {
              user_id: function() {
                  return "{{ $user_id }}";
              }
